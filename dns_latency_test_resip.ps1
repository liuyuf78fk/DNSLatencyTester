...
while ($true) {
    ...
    Write-Host "`n=== Starting DNS latency test (Response IP address) for: $domain ===`n"

    foreach ($dns in $dnsServers) {
        Write-Host "Testing DNS: $($dns.Name) ($($dns.IP)) ..." -NoNewline
        try {
            $start = Get-Date
            $records = Resolve-DnsName -Name $domain -Server $dns.IP -ErrorAction Stop
            $end = Get-Date
            $ms = [math]::Round(($end - $start).TotalMilliseconds, 2)

            # Get IPs from A and AAAA records
            $ips = ($records | Where-Object { $_.Type -in @('A', 'AAAA') }).IPAddress -join ', '
            if (-not $ips) { $ips = "No A/AAAA records" }

            Write-Host " $ms ms | IP: $ips" -ForegroundColor Green
        }
        catch {
    ...
    # If Enter, do nothing – repeat loop with current domain
}